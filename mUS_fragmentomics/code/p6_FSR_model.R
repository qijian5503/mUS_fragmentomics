...
# save(limma_de_all,file="medip_brca_FSR_samp135_limma_PSM_de_all.RData")


load("medip_brca_FSR_samp135_limma_PSM_de_all.RData")
all_de2=data.frame()
for (r in 1:100) {
    res=limma_de_all[[r]]
...
FSR_gene=de_gene[!grepl("chrX|chrY",de_gene)]

library(glmnet)
FSR_de_exp=as.data.frame(t(dat[FSR_gene,!colnames(dat) %in% valid_sample]))
FSR_de_exp$Group=phen_brca[rownames(FSR_de_exp),"Group"]
cv_fit = cv.glmnet(as.matrix(FSR_de_exp[,FSR_gene]), FSR_de_exp$Group, family = "binomial",type.measure = "class")
coef.min = coef(cv_fit, s = "lambda.min")
FSR_gene2=coef.min@Dimnames[[1]][coef.min@i+1][-1]  ## 筛选得到的基因
# save(cv_fit,file = "FSR_lasso_cv_fit.RData")

FSR_de_exp=as.data.frame(t(dat[FSR_gene2,]))
FSR_de_exp$group=phen_brca[rownames(FSR_de_exp),"Group"]
FSR_de_exp$group=factor(FSR_de_exp$group,levels = c("Normal","Tumor"))

limma_modlist=list()
for(i in 1:100){
    train_samp=subsample[[i]]$train_samp
    train_samp=train_samp[train_samp %in% rownames(FSR_de_exp)]
    test_samp=subsample[[i]]$test_samp
    test_samp=test_samp[test_samp %in% rownames(FSR_de_exp)]
    train_data=FSR_de_exp[rownames(FSR_de_exp) %in% train_samp,c(FSR_gene2,"group")]
    test_data=FSR_de_exp[rownames(FSR_de_exp) %in% test_samp,c(FSR_gene2,"group")]
    valid_data=FSR_de_exp[rownames(FSR_de_exp) %in% valid_sample,c(FSR_gene2,"group")]
...
# save(limma_modlist,file="medip_brca_FSR_samp135_rf_modlist.RData") 
